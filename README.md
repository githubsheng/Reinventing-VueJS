...
In our first example, `vm.authorId` isn’t a dependency of our root SV component, but its changes causes the root component to update. In our second example, `data.production.dimension.height` isn’t a dependency of the parent component, but its changes causes the parent component to update.
...
Todo: insert code: inside property getter, record which component is being rendered.
